...
public final class AwsConnection {
	...
	/**
	 * @return instance of connection to AWS that caches results set for a
	 *         certain time.
	 */
	...
	/**
	 * @param collectFromAws
	 *            automatically collect usage data from AWS.
	 * @return filled statistics object containing all used resources.
	 */
	public AwsStats getStats(boolean collectFromAws) {
		if ((stats == null) || (stats.getAllResources().length == 0) || ((DateTime.now().getMillis() - lastCollectTime.getMillis()) > cacheDuration)) {
